...
def data_preprocessing(task_1a_dataframe):
	...
	#################	ADD YOUR CODE HERE	##################    
    # Perform one-hot encoding for 'City' and 'Gender'
    encoded_dataframe = pandas.get_dummies(task_1a_dataframe, columns=['City', 'Gender'])

    # Label encode 'Education'
    le = LabelEncoder()
    encoded_dataframe['Education'] = le.fit_transform(task_1a_dataframe['Education'])

    # Convert 'EverBenched' to binary (0 or 1)
    encoded_dataframe['EverBenched'] = (encoded_dataframe['EverBenched'] == 'Yes').astype(int)
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
	...
	'''	
	#################	ADD YOUR CODE HERE	##################
    X_train_tensor, _, y_train_tensor, _ = tensors_and_iterable_training_data
    batch_size = 200
    for epoch in range(number_of_epochs):
       ...
       #cond print(f"Epoch {epoch + 1}/{number_of_epochs}, Loss: {total_loss / (len(X_train_tensor) / batch_size)}")
    trained_model=model
	##########################################################
...
def validation_function(trained_model, tensors_and_iterable_training_data):
	...
	'''	
	#################	ADD YOUR CODE HERE	##################
    _, X_test_tensor, _, y_test_tensor = tensors_and_iterable_training_data
    trained_model.eval()
    with torch.no_grad():
